...
async function showMsg() {
    ...
    if ($.beanCount) {
        ReturnMessage += `【当前京豆】${$.beanCount - $.beanChangeXi}豆(≈${(($.beanCount - $.beanChangeXi) / 100).toFixed(2)}R)\n`;
        strsummary += `【当前京豆】${$.beanCount - $.beanChangeXi}豆(≈${(($.beanCount - $.beanChangeXi) / 100).toFixed(2)}R)\n`;
    } else {
        if ($.levelName || $.JingXiang)
    ...
    ReturnMessage += `【当前喜豆】${$.xibeanCount}喜豆(≈${($.xibeanCount / 100).toFixed(2)}R)\n`;
    strsummary += `【当前喜豆】${$.xibeanCount}豆(≈${($.xibeanCount / 100).toFixed(2)}R)\n`;
    ...
    if ($.JDtotalcash) {
        ReturnMessage += `【特价金币】${$.JDtotalcash}币(≈${($.JDtotalcash / 10000).toFixed(2)}R)\n`;
    }
    if ($.JdzzNum) {
        ReturnMessage += `【京东赚赚】${$.JdzzNum}币(≈${($.JdzzNum / 10000).toFixed(2)}R)\n`;
    }
    if ($.JdMsScore != 0) {
        ReturnMessage += `【京东秒杀】${$.JdMsScore}币(≈${($.JdMsScore / 1000).toFixed(2)}R)\n`;
    }
    if ($.ECardinfo)
    ...
    if ($.joylevel || $.jdCash || $.JoyRunningAmount) {
        ...
        if ($.jdCash) {
            ...
            ReturnMessage += `领现金:${$.jdCash}R`;
        }
        if ($.JoyRunningAmount) {
            if ($.joylevel || $.jdCash) {
                ReturnMessage += ",";
            }
            ReturnMessage += `汪汪赛跑:${$.JoyRunningAmount}R`;
        }
    ...
    if (!llPetError && initPetTownRes) {
        if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
            ...
            } else if (responsec.resultCode === '0') {
    ...
    if ($.quanpin){
        ReturnMessage += `${$.quanpin}`
    }
    if ($.YunFeiQuan){
        ReturnMessage += `【运费券】${$.YunFeiTitle},${$.YunFeiQuan}张，到期时间:${$.YunFeiQuanEndTime}\n`;
    }
            ...
            try {
                if (err) {
                    ...
                    if (useable) {
                        ...
                        if (balEcard)
                            $.ECardinfo = '共' + useable.length + '张E卡,合计' + parseFloat(balEcard).toFixed(2) + 'R';
                    }
...
async function getJingBeanBalanceDetail1(page) {
    let functionId = 'getJingBeanBalanceDetail'
    let uuid = "87" + randomnum(13) + "-53" + randomnum(10)
    let st = Date.now()
    let sv = '111'
    let client = 'android'
    let clientVersion = '11.2.8'
    let dbody = JSON.stringify({page: page, pageSize: 20})
    const options = {
        url: 'http://127.0.0.1:3500/api/getsign',
        body: JSON.stringify({
    ...
    let sign = await reqpost(options)
    console.log(sign)
    return new Promise(async resolve => {
        const options = {
            "url": 'https://api.m.jd.com/client.action?functionId=getJingBeanBalanceDetail&clientVersion=11.2.8&uuid=' + uuid + '&st=' + st + '&sign=' + sign + '&sv=111&client=android&area=13_1072_2926_29094',
            "body": `lmt=0&body=${encodeURIComponent(dbody)}`,
            "headers": {
                'User-Agent': $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"),
                'Host': 'api.m.jd.com',
                'Content-Type': 'application/x-www-form-urlencoded',
            ...
            try {
                if (err) {
                    ...
                    if (data) {
                        ...
                        if ($.jxRed > 0)
                            $.message += `【京喜红包】${$.jxRed}(将过期${$.jxRedExpire.toFixed(2)})R \n`;
                        if ($.jsRed > 0)
                            $.message += `【特价红包】${$.jsRed}(将过期${$.jsRedExpire.toFixed(2)})R \n`;
                        if ($.jdRed > 0)
...
function getCoupon() {
    return new Promise(async resolve => {
        let options = {
            url: `https://wq.jd.com/activeapi/queryjdcouponlistwithfinance?state=1&wxadd=1&filterswitch=1&_=${Date.now()}&sceneval=2&callback=jsonpCBKB&g_ty=ls`,
            headers: {
                'authority': 'wq.jd.com',
        ...
        $.get(options, async (err, resp, data) => {
            try {
                ...
                // console.log(data.coupon.useable)
                // for (let i = 0; i < data.coupon.useable.length; i++) {
                //     console.log(data.coupon.useable[i].couponTitle)
                //     console.log(data.coupon.useable[i].limitStr)
                //     console.log("\n")
                // }
                ...
                for (let i = 0; i < useable.length; i++) {
                    ...
                    if (useable[i].limitStr.indexOf('全品类') > -1) {
                        ...
                        if ($.beginTime < new Date().getTime() && useable[i].quota < 20 && useable[i].coupontype === 1) {
                            ...
                            $.quanpin += `【全品类券】满${decquota}减${decdisc}R`;

                            if (useable[i].endTime < $.todayEndTime) {
                                $.quanpin += `(今日过期,${$.platFormInfo})\n`;
                            } else if (useable[i].endTime < $.tomorrowEndTime) {
                                $.quanpin += `(明日将过期,${$.platFormInfo})\n`;
                            } else {
                                $.quanpin += `(${$.platFormInfo})\n`;
                            }
                    ...
                    if (useable[i].couponTitle.indexOf('运费券') > -1 && useable[i].limitStr.indexOf('自营商品运费') > -1) {
                        if (!$.YunFeiTitle) {
                            let yftitle = useable[i].couponTitle.slice(0,useable[i].couponTitle.indexOf('运费券')+3)
                            $.YunFeiTitle = yftitle;
                            $.YunFeiQuanEndTime = new Date(parseInt(useable[i].endTime)).Format('yyyy-MM-dd');
                            $.YunFeiQuan += 1;

                        } else {
                            if ($.YunFeiTitle === useable[i].couponTitle.slice(0,useable[i].couponTitle.indexOf('运费券')+3)) {
                                $.YunFeiQuanEndTime = new Date(parseInt(useable[i].endTime)).Format('yyyy-MM-dd');
                                $.YunFeiQuan += 1;
                            } else {
                                if (!$.YunFeiTitle2)
                                    $.YunFeiTitle2 = useable[i].couponTitle.slice(0,useable[i].couponTitle.indexOf('运费券')+3);

                                if ($.YunFeiTitle2 === useable[i].couponTitle.slice(0,useable[i].couponTitle.indexOf('运费券')+3)) {
                    ...
                    if (useable[i].couponTitle.indexOf('京东特价APP活动') > -1 && useable[i].limitStr == '仅可购买活动商品') {
                        $.beginTime = useable[i].beginTime;
                        if ($.beginTime < new Date().getTime() && useable[i].coupontype === 1) {
                            ...
                            $.message += `【京东特价券】满${decquota}减${decdisc}R`;
                    ...
                    if (useable[i].couponStyle == 7 || useable[i].couponStyle == 8) {
                        ...
                        if (useable[i].discount < useable[i].quota)
                            $.message += `【${$.couponType}】满${useable[i].quota}减${useable[i].discount}R`;
                        else
                            $.message += `【${$.couponType}】立减${useable[i].discount}R`;
                        if (useable[i].platFormInfo)
                            $.platFormInfo = useable[i].platFormInfo;
...
// async function wangwangleyuan() {
//     let opts = {
//         url: `https://api.m.jd.com/?functionId=joyList&body={%22linkId%22:%22LsQNxL7iWDlXUs6cFl-AAg%22}&t=1663076971464&appid=activities_platform&h5st=20220913214931488%3B2488851706439259%3Be18ed%3Btk02w93181b7a18nQ4ElqaFrbtTS7MpqmcnwL%2Fg%2BR6fJk4FaFCWH3WJZDi81KAlw7NTe23dn1%2FcLkNVp1GnWfi9mwHMl%3B05c30f5c9edd4729228b91b0fb4c4b60d11e6c5a022606dcc053a1778301af46%3B3.1%3B1663076971488%&cthr=1`,
//         headers: {
//             'cookie': cookie,
//             'user-agent': 'jdpingou;iPhone;4.13.0;14.4.2;${randomString(40)};network/wifi;model/iPhone10,2;appBuild/100609;supportApplePay/1;hasUPPay/0;pushNoticeIsOpen/1;hasOCPay/0;supportBestPay/0;session/${Math.random * 98 + 1};pap/JA2019_3111789;brand/apple;supportJDSHWK/1;Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148',
//             'referer': 'https://joypark.jd.com/',
...
!function (t, e) {
    ...
    var h, t, e, r, i, n, f, o, s, c, a, l, d, m, x, b, H, z, A, u, p, _, v, y, g, B, w, k, S, C, D, E, R, M, F, P, W,
        O, I, U, K, X, L, j, N, T, q, Z, V, G, J, $, Q, Y, tt, et, rt, it, nt, ot, st, ct, at, ht, lt, ft, dt, ut, pt,
        _t, vt, yt, gt, Bt, wt, kt, St, bt = bt || function (l) {
            var t;
            ...
            var e = {}, o = e.lib = {}, s = o.Base = {
                ...
                reset: function () {
                    ...
                    var e, r = this._data, i = r.words, n = r.sigBytes, o = this.blockSize, s = n / (4 * o),
                        c = (s = t ? l.ceil(s) : l.max((0 | s) - this._minBufferSize, 0)) * o, a = l.min(4 * c, n);
                    if (c) {
    ...
    return h = bt.lib.WordArray, bt.enc.Base64 = {
        ...
        var t = bt, e = t.lib, r = e.WordArray, i = e.Hasher, n = t.algo, H = [];
        !function () {
        ...
        var o = n.MD5 = i.extend({
            _doReset: function () {
                ...
                var o = this._hash.words, s = t[e + 0], c = t[e + 1], a = t[e + 2], h = t[e + 3], l = t[e + 4],
                    f = t[e + 5], d = t[e + 6], u = t[e + 7], p = t[e + 8], _ = t[e + 9], v = t[e + 10], y = t[e + 11],
                    g = t[e + 12], B = t[e + 13], w = t[e + 14], k = t[e + 15], S = o[0], m = o[1], x = o[2], b = o[3];
                ...
                e[i >>> 5] |= 128 << 24 - i % 32;
                var n = l.floor(r / 4294967296), o = r;
    ...
    }), t.SHA1 = i._createHelper(o), t.HmacSHA1 = i._createHmacHelper(o), function (n) {
        var t = bt, e = t.lib, r = e.WordArray, i = e.Hasher, o = t.algo, s = [], B = [];
        !function () {
            function t(t) {
        ...
        var n = bt.lib.WordArray, t = bt.enc;
        t.Utf16 = t.Utf16BE = {
            stringify: function (t) {
        ...
        if ("function" == typeof ArrayBuffer) {
            var t = bt.lib.WordArray, n = t.init;
            (t.init = function (t) {
        ...
        _doReset: function () {
            ...
            var o, s, c, a, h, l, f, d, u, p, _, v = this._hash.words, y = z.words, g = A.words, B = m.words,
                w = x.words, k = b.words, S = H.words;
        ...
        var t = bt, e = t.lib, u = e.WordArray, i = e.Hasher, l = t.x64.Word, r = t.algo, C = [], D = [], E = [];
        !function () {
            ...
            cfg: i.cfg.extend({outputLength: 512}), _doReset: function () {
                ...
                for (var c = 0; c < 24; c++) {
                    ...
                    for (a = 0; a < 5; a++) for (f = 0; f < 5; f++) {
                        var x = r[y = a + 5 * f], b = R[y], H = R[(a + 1) % 5 + 5 * f], z = R[(a + 2) % 5 + 5 * f];
        ...
        var t = bt, e = t.lib.Hasher, r = t.x64, i = r.Word, n = r.WordArray, o = t.algo;
        ...
        var c = o.SHA512 = e.extend({
            _doReset: function () {
                ...
                for (var r = this._hash.words, i = r[0], n = r[1], o = r[2], s = r[3], c = r[4], a = r[5], h = r[6], l = r[7], f = i.high, d = i.low, u = n.high, p = n.low, _ = o.high, v = o.low, y = s.high, g = s.low, B = c.high, w = c.low, k = a.high, S = a.low, m = h.high, x = h.low, b = l.high, H = l.low, z = f, A = d, C = u, D = p, E = _, R = v, M = y, F = g, P = B, W = w, O = k, I = S, U = m, K = x, X = b, L = H, j = 0; j < 80; j++) {
                    var N, T, q = xt[j];
                    if (j < 16) T = q.high = 0 | t[e + 2 * j], N = q.low = 0 | t[e + 2 * j + 1]; else {
                        var Z = xt[j - 15], V = Z.high, G = Z.low,
                            J = (V >>> 1 | G << 31) ^ (V >>> 8 | G << 24) ^ V >>> 7,
                            $ = (G >>> 1 | V << 31) ^ (G >>> 8 | V << 24) ^ (G >>> 7 | V << 25), Q = xt[j - 2],
                            Y = Q.high, tt = Q.low, et = (Y >>> 19 | tt << 13) ^ (Y << 3 | tt >>> 29) ^ Y >>> 6,
                            rt = (tt >>> 19 | Y << 13) ^ (tt << 3 | Y >>> 29) ^ (tt >>> 6 | Y << 26), it = xt[j - 7],
                            nt = it.high, ot = it.low, st = xt[j - 16], ct = st.high, at = st.low;
                        T = (T = (T = J + nt + ((N = $ + ot) >>> 0 < $ >>> 0 ? 1 : 0)) + et + ((N += rt) >>> 0 < rt >>> 0 ? 1 : 0)) + ct + ((N += at) >>> 0 < at >>> 0 ? 1 : 0), q.high = T, q.low = N
                    }
                    var ht, lt = P & O ^ ~P & U, ft = W & I ^ ~W & K, dt = z & C ^ z & E ^ C & E,
                        ut = A & D ^ A & R ^ D & R,
                        pt = (z >>> 28 | A << 4) ^ (z << 30 | A >>> 2) ^ (z << 25 | A >>> 7),
                        _t = (A >>> 28 | z << 4) ^ (A << 30 | z >>> 2) ^ (A << 25 | z >>> 7),
                        vt = (P >>> 14 | W << 18) ^ (P >>> 18 | W << 14) ^ (P << 23 | W >>> 9),
                        yt = (W >>> 14 | P << 18) ^ (W >>> 18 | P << 14) ^ (W << 23 | P >>> 9), gt = mt[j],
                        Bt = gt.high, wt = gt.low, kt = X + vt + ((ht = L + yt) >>> 0 < L >>> 0 ? 1 : 0), St = _t + ut;
        ...
        var t = bt, e = t.lib, r = e.Base, a = e.WordArray, i = e.BufferedBlockAlgorithm, n = t.enc,
            o = (n.Utf8, n.Base64), s = t.algo.EvpKDF, c = e.Cipher = i.extend({
        ...
        var _ = (t.pad = {}).Pkcs7 = {
            ...
            cfg: c.cfg.extend({mode: u, padding: _}), reset: function () {
                var t;
                c.reset.call(this);
        ...
        var t = bt, e = t.lib.BlockCipher, r = t.algo, h = [], l = [], f = [], d = [], u = [], p = [], _ = [], v = [],
            y = [], g = [];
        !function () {
        ...
        var t = bt, e = t.lib, n = e.WordArray, r = e.BlockCipher, i = t.algo,
            ...
            }], u = [4160749569, 528482304, 33030144, 2064384, 129024, 8064, 504, 2147483679], o = i.DES = r.extend({
                _doReset: function () {
        ...
        var t = bt, e = t.lib.StreamCipher, r = t.algo, i = r.RC4 = e.extend({
            _doReset: function () {
...
module.exports.getChangePro = getChangePro
